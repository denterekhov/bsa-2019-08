  ...
  parse (path) {
    const
      contents = this.readFile(path);
			const validationErrors = this.validate(contents);
    ...
    const
      lines = contents.split(/\n/).filter((l) => l).filter((l, i) => i > 0);
			const items = lines.map(l => this.parseLine(l));
  ...
  validate (contents) {
    const
      errors = [];
			...
			const bodyLines = lines.filter((l, i) => i > 0);
    ...
    for (let i = 0; i < bodyLines.length; i++) {
      ...
      for (let j = 0; j < this.schema.columns.length; j++) {
        const
          cell = cells[j];
					const columnType = this.schema.columns[j].type;
  ...
  parseLine (csvLine) {
    const
      columns = this.schema.columns;
			...
			const values = csvLine.split(/,/).map(cell => cell.trim());
			const item = {};
    ...
    for (let i = 0; i < keys.length; i++) {
      const
        type = types[i];
				...
				const valueTyped = type === this.ColumnType.NUMBER_POSITIVE ? Number(value) : value;
